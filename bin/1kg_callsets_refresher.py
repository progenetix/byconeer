...
def callsets_refresher():
    initialize_bycon_service(byc, "callsets")
    ...
    #
    # print("Re-generating statusmaps with {} intervals for {} callsets...".format(len(byc["genomic_intervals"]),
    #                                                                              len(cs_ids)))

    #no = len(cs_ids)
    my_cs = data_db["callsets"].find({"biosample_id" : {'$regex': "onekg"}})
    no = len(list(my_cs))
    ...
    #
    #     bar.next()
    for document in data_db["callsets"].find({"biosample_id" : "onekgbs-HG00100"}):
        cs_ids.append(document['id'])
    #cs_ids = ['onekgcs-HG00100','onekgcs-HG00101']
    my_cs = data_db["callsets"].find({"biosample_id" : "onekgbs-HG00100"})
    for id in cs_ids:
        
        cs = cs_coll.find_one({"id": id})
        csid = id
        print(csid)
        ...
        if not byc["test_mode"]:
            cs_coll.update_one({"id": id}, {'$set': cs_update_obj})
        else:
            prjsonnice(maps)
